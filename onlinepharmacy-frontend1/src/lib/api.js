...
// ✅ Delete drug (requires admin username as query param)
...
// ✅ Update drug (also requires admin username as query param)
export async function updateDrug(drugId, drug, adminUsername) {
  const res = await api.put(`/api/drugs/${drugId}`, drug, {
    params: { adminUsername }  // ✅ match backend @RequestParam name
  });
  return res.data;
}
